...
main_body:
  ...
  %"time_call" = call i64 @"time"(i64 0)
  %"seed" = alloca i64
  store i64 %"time_call", i64* %"seed"
  %"seed_load" = load i64, i64* %"seed"
  %"icmp" = icmp slt i64 %"seed_load", 0
  br i1 %"icmp", label %"if_then", label %"if_else"
if_then:
  %"seed_load.1" = load i64, i64* %"seed"
  %"neg" = sub i64 0, %"seed_load.1"
  store i64 %"neg", i64* %"seed"
  br label %"if_end"
if_else:
  br label %"if_end"
if_end:
...
while_body:
  %"seed_load.2" = load i64, i64* %"seed"
  %"mul" = mul i64 %"seed_load.2", 1103515245
  %"add" = add i64 %"mul", 12345
  %"bitand" = and i64 %"add", 2147483647
  store i64 %"bitand", i64* %"seed"
  %"seed_load.3" = load i64, i64* %"seed"
  %"mod" = srem i64 %"seed_load.3", 512
  %"add.1" = add i64 %"mod", 1
  %"size" = alloca i64
  store i64 %"add.1", i64* %"size"
  %"size_load" = load i64, i64* %"size"
  %"alloc_bytes_call" = call i8* @"GC_malloc"(i64 %"size_load")
  %"p" = alloca i8*
